...
const AdminScreen = () => {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm("Are you sure you want to delete this document? All associated data will be removed.")) return;
    ...
    <div className="max-w-5xl mx-auto p-6">
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Upload PDFs to train your chatbot context.</p>
        </div>
                    ...
                    <div className="flex items-center gap-2">
                      {doc.status === 'processed' && (
                        <>
                          ...
                          <span className="text-green-700 text-sm font-medium">Processed</span>
                        </>
                      )}
                      {doc.status === 'processing' && (
                        <>
                          <Clock className="w-4 h-4 text-yellow-500 animate-pulse" />
                          <span className="text-yellow-700 text-sm font-medium">Processing...</span>
                        </>
                      )}
                      {doc.status === 'error' && (
                        <>
